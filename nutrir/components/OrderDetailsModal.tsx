  ...
  return (
    <>
      ...
      <div
        role="dialog"
        aria-modal="true"
        className="fixed left-1/2 top-1/2 z-[90] flex max-h-[min(90vh,640px)] w-[min(92vw,440px)] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl bg-nutrir-canvas-alt shadow-2xl"
      >
        <div className="flex items-start justify-between gap-3 border-b border-nutrir-nude-dark/40 px-5 py-4">
          <div>
            <h2 className="font-display text-xl font-bold text-nutrir-ink">Detalhes do pedido</h2>
            <p className="mt-1 text-sm text-nutrir-ink/65">
        ...
        <ul className="flex-1 space-y-3 overflow-y-auto px-5 py-4">
          {order.items.map((item, index) => {
            ...
            return (
              ...
              >
                <p className="font-display text-base font-bold text-nutrir-ink">{display.title}</p>
                ...
                {display.addonsNote && (
                  <p className="mt-2 whitespace-pre-line text-xs leading-relaxed text-nutrir-ink/60">
                    {display.addonsNote}
                  </p>
                )}
              </li>
