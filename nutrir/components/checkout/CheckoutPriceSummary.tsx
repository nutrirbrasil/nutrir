...
import { isOnlineCardPayment, isOnlinePixPayment } from "@/lib/payment-utils";
...
export function CheckoutPriceSummary({ pricing, method, compact = false }: Props) {
  ...
  return (
    <div className={compact ? "space-y-2" : "space-y-3"}>
      ...
      {!compact && isOnlinePixPayment(method) && (
        <p className="rounded-lg bg-nutrir-emerald/5 p-3 text-sm text-nutrir-emerald/90">
          Na próxima tela você verá o QR Code e o Pix copia e cola para pagar.
        </p>
      )}

      {!compact && isOnlineCardPayment(method) && (
        <p className="rounded-lg bg-nutrir-emerald/5 p-3 text-sm text-nutrir-emerald/90">
          Você será redirecionado ao checkout seguro InfinitePay para pagar com cartão.
        </p>
      )}
    </div>
  );
